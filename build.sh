...
for _PREFIX in 'x86_64-w64-mingw32-' 'i686-w64-mingw32-'; do
	...
	"${_PREFIX}g++${_SUFFIX}" -DUNICODE -D_UNICODE -mwin32 -municode --static -static-libgcc -static-libstdc++ $(grep RelativePath updlist.vcproj | grep -o '".*"' | grep -vE '(ReadMe.txt|.rc|.ico)"' | sed -ze 's/".\\//g; s/\"\n/ /g') "${_PREFIX}updlist${_SUFFIX}.res" -o "${_OUTPUT}" -l version -l shlwapi -l crypt32
done
#wait
