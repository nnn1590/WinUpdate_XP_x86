...
for _PREFIX in 'x86_64-w64-mingw32-' 'i686-w64-mingw32-'; do
	...
	"${_PREFIX}g++${_SUFFIX}" -specs="${_PREFIX}g++${_SUFFIX}.specs" -DUNICODE -D_UNICODE -municode --static -static-libgcc -static-libstdc++ $(grep RelativePath updlist.vcproj | grep -o '".*"' | grep -vE '(ReadMe.txt|.rc|.ico)"' | sed -ze 's/".\\//g; s/\"\n/ /g') "${_PREFIX}updlist${_SUFFIX}.res" -o "${_OUTPUT}" -lversion -lshlwapi -lcrypt32 -lmsvcr120
done
#wait
